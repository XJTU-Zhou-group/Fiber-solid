...
fiber_length_solid = 0.105
fiber_radius_solid = 0.0035
...
fiber_Elastic = 230000000
fiber_Poisson_ratio = 0.25
matrix_Elastic = 44800000
...
while fiber_real_num < fiber_num_solid:
    ...
    y = random.uniform(fiber_radius_solid, width - fiber_radius_solid)
    ...
    x2 = x + fiber_length_solid*sin(radians(angle_y))*cos(radians(angle_z))
    y2 = y + fiber_length_solid*sin(radians(angle_y))*sin(radians(angle_z))
    ...
    if fiber_radius_solid <= x2 <= length - fiber_radius_solid and fiber_radius_solid <= y2 <= width - fiber_radius_solid and fiber_radius_solid <= z2 <= height - fiber_radius_solid:
        ...
        if len(points) == 0 or interact_judgement(points, newpoint, fiber_radius_solid):
            ...
            pass
    elif x2 < 0 or y2 < 0 or z2 < 0 or x2 > length or y2 > width or z2 > height:
        sign = True
...
import easypbc
easypbc.feasypbc(part='Model-1', inst='Part-base-1', meshsens=1E-07, CPU=20,
