...
fiber_length_solid = 1
fiber_radius_solid = 0.0125
fiber_volume_solid = 0.3
fiber_num_solid = int((fiber_volume_solid * length * width * height)/(3.14159 * fiber_length_solid * fiber_radius_solid ** 2))
...
while fiber_real_num < fiber_num_solid:
    ...
    z2 = z + fiber_length_solid
    x2 = x
    y2 = y

    if fiber_radius_solid <= x2 <= length - fiber_radius_solid and fiber_radius_solid <= y2 <= width - fiber_radius_solid and 0 <= z2 <= height:
        ...
        if len(points) == 0 or interact_judgement(points, newpoint, fiber_radius_solid):
            ...
            pass
    elif x2 < fiber_radius_solid or y2 < fiber_radius_solid or z2 < 0 or x2 > length - fiber_radius_solid or y2 > width - fiber_radius_solid or z2 > height:
        sign = True
...
p.generateMesh()
execfile('D:/W-sample/jihe/voxel/voxel-fiber.py', __main__.__dict__)
...
# EasyPBC
import sys
sys.path.insert(8, r'c:/Users/dell/abaqus_plugins/EasyPBC V.1.4')
import easypbc
easypbc.feasypbc(part='Model-1', inst='Part-base-1', meshsens=1E-07, CPU=55,
